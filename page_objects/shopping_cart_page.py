...
class ShoppingCartPage(BasePage):
    ...
    def verify_item_spec(self, spec):
        assert spec in self.find_element(self.ITEM_SPECS["locator"]).text, f"Item spec {spec} not in item description"

    def click_proceed_to_checkout(self):
        self.is_visible(self.PROCEED_TO_CHECKOUT_BTN["locator"])
        self.assert_text_in_element(self.PROCEED_TO_CHECKOUT_BTN["locator"], self.PROCEED_TO_CHECKOUT_BTN["text"])
